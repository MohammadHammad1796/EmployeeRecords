...
function AddRow(table, file) {
  const rowClass = $(table).find("tr:last").hasClass("odd") ? "even" : "odd";

  $(table)
    .find("tbody:eq(0)")
    .append($(`<tr id="item_${file.id}" class="${rowClass}"></tr>`));
  $(table).find("tr:last").append($(`<td head="Name">`).text(file.name));
  $(table)
    .find("tr:last")
    .append($(`<td head="Size">`).text(`${file.size} Mb`));
  const fileExtension = file.path.split(".").pop().toLowerCase();
  switch (fileExtension) {
    case "pdf":
      $(table)
        .find("tr:last")
        .append(`<td head='Path'><a><i class="fas fa-file-pdf"></i></a></td>`);
      break;
    case "dot":
    case "doc":
    case "docx":
      $(table)
        .find("tr:last")
        .append(`<td head='Path'><a><i class="fas fa-file-word"></i></a></td>`);
      break;
    default:
      $(table)
        .find("tr:last")
        .append(`<td head='Path'><img src="/${file.path}"/></td>`);
  }
  $(table).find("tr:last").append(`<td head="Actions"></td>`);
  $(table).find("tr:last td:last")
    .append(`<a href="/${file.path}"><i class="fas fa-save me-3 mb-3"></i></a>
  ...
  $(table)
    .find("tr:last td:last a:eq(0)")
    .attr("download", `${employeeName} - ${file.name}.${fileExtension}`);
}
      ...
      $.ajax({
        ...
        statusCode: {
          ...
          200: function (response) {
            const fileSize = ConvertFileSizeFromByteToMegaByte(
              saveResource.get("File").size
            ).toFixed(2);
...
function GetAndRenderEmployeeInfo() {
  ...
  if (employee == null) window.location.href = "/notFound";

  employeeName = employee.name;
  $("#info").html(`<div class="row"></div>`);
  $("#info .row").append(`<div class="col-md-3 col-sm-6 col-12"></div>`);
  $("#info .row div:last").append($("<p>").text(`Name: ${employee.name}`));
  $("#info .row").append(`<div class="col-md-3 col-sm-6 col-12"></div>`);
  $("#info .row div:last").append(
    $("<p>").text(`Department: ${employee.department.name}`)
  );
  $("#info .row").append(`<div class="col-md-3 col-sm-6 col-12"></div>`);
  $("#info .row div:last").append(
    $("<p>").text(`Date of birth: ${employee.dateOfBirth}`)
  );
  $("#info .row").append(`<div class="col-md-3 col-sm-6 col-12"></div>`);
  $("#info .row div:last").append(
    $("<p>").text(`Address: ${employee.address}`)
  );
}
...
$(
  (function () {
    jQuery.validator.addMethod("maximumFileSizeIfHaveVale", function (value) {
      ...
      var isValid = true;
      if (sizeInMb > 5) isValid = false;

      return isValid;
    });

    jQuery.validator.unobtrusive.adapters.add(
      "maximumFileSizeIfHaveVale",
      [],
      function (options) {
