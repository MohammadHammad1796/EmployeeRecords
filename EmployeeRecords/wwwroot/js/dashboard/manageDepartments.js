...
function AddRow(table, department) {
  const rowClass = $(table).find("tr:last").hasClass("odd") ? "even" : "odd";

  $(table)
    .find("tbody:eq(0)")
    .append($(`<tr id="item_${department.id}" class="${rowClass}"></tr>`));
  $(table).find("tr:last").append($(`<td head="Name">`).text(department.name));
  $(table).find("tr:last").append(`<td head="Actions">`);
  $(table).find("tr:last td:last")
    .append(`<a href="#" class="edit-btn" data-id="${department.id}"><i class="fas fa-edit me-3 mb-3"></i></a>
