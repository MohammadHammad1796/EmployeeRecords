...
function AddRow(table, employee) {
  ...
  $(table)
    .find("tbody:eq(0)")
    .append($(`<tr id="item_${employee.id}" class="${rowClass}"></tr>`));
  $(table).find("tr:last").append($(`<td head="Name">`).text(employee.name));
  $(table)
    .find("tr:last")
    .append(
      $(`<td head="Department" data-id="${employee.department.id}">`).text(
        employee.department.name
      )
    );
  $(table)
    .find("tr:last")
    .append($(`<td head="Date of birth">`).text(employee.dateOfBirth));
  $(table)
    .find("tr:last")
    .append($(`<td head="Address">`).text(employee.address));
  $(table).find("tr:last").append(`<td head="Actions">`);
  $(table).find("tr:last td:last")
    .append(`<a href="/employees/${employee.id}/files" target="_blank"><i class="fas fa-file-archive me-3 mb-3"></i></a>
...
function ConfigureEdit() {
  $("#items").on("click", "a.edit-btn", function (e) {
    ...
    $("#manageModalLabel").text(`Edit employee: ${employeeName}`);
...
function ConfigureDelete(table, id) {
  $(table).on("click", "a.remove-btn", function (e) {
    ...
    $(
      "#deleteParagraph"
    ).html(`<p>Are you sure you want to delete this employee?</p>
		<div class="col-12"></div>`);
    $("#deleteParagraph div:eq(0)").append($("<div>").text(`Name: ${name}`));
    $("#deleteParagraph div:eq(0)").append(
      $("<div>").text(`Department: ${department}`)
    );
    $("#deleteParagraph div:eq(0)").append(
      $("<div>").text(`Date of birth: ${dateOfBirth}`)
    );
    $("#deleteParagraph div:eq(0)").append(
...
$(window).on("load", function () {
  ...
  $("#departmentName").on("blur keyup paste keydown", function () {
    if (
      selectedDepartment !== null &&
      $(this).val().trim() !== selectedDepartment.name
    )
      $("#DepartmentId").val("");
  });
